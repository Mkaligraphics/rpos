...
function Header()
{
   $image = '../img/';
    ...
    $this->Cell(80,4,'Utensils reports',0,0,'C');
...
$this->Line(20, 45, 210-20, 45); // 20mm from each edge
    ...
    $this->Cell(108,7,'Descriptions',1,0,'C',true);    
    $this->Cell(80,7,'Quantity',1,1,'C',true);
...
$gbs->SetDrawColor(0, 0, 0);
  ...
  $qry = $data->con->query("SELECT * FROM utensils WHERE active = '1'");
  while($rws = mysqli_fetch_array($qry)){

                    $gbs->Cell(108,6,$rws['description'],1,0,'C',true);                    
                    $gbs->Cell(80,6,$rws['quantity'],1,1,'C',true);  
